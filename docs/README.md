...
- **カレンダーを一括登録**: カレンダー一覧の「📄 CSVインポート」→ `url,name,color` のヘッダー行を持つ CSV（name/color は省略可、カンマ/タブ両対応）を貼り付けまたはファイル選択。プレビューで行ごとの妥当性（URL は `https://` / `webcal://` のみ）を確認してから追加します。追加はブラウザ内の一時状態への反映のみで、実際に保存するには「プロファイルを保存」を押してください。
